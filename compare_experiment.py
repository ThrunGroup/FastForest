...
from fit_heart import split_data, load_data
...
def experiment_cf(t: int, seed: int, p_value: float = 0.95, verbose: bool = False) -> bool:
    ...
    for i in range(t):
        ...
        print(np.unique(sk_forest.predict(X_test)))
        print(f_forest.predict_batch(X_test)[0])
