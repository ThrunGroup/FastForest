...
import exp_utils
from exp_utils import get_datasets, compute_RF_accuracy, get_other_split_log_file_filepath
...
def aggregate_results(other_split : str, nls_pure_results : dict, ls_pure_results: dict) -> Tuple[Sequence[str], Sequence[float]]:
    ...
    train_speedup_res = other_split + " trains forest " + str(train_speedup) + "% faster"
    train_eval_speedup_res = other_split + " evals on train set " + str(train_eval_speedup) + "% faster"
    test_eval_speedup_res  = other_split + " evals on test set " + str(test_eval_speedup) + "% faster"
    train_acc_gain_res = other_split + " gains " + str(train_acc_gain) + "% on training accuracy"
    test_acc_gain_res = other_split + " gains " + str(test_acc_gain) + "% on training accuracy"
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the other split method to compare aganist LINEAR splitting",
    )
    ...
    print("STARTING LINEAR EXPERIMENT")
    ...
    print("STARTING " + args.other_split + " SPLITTING EXPERIMENT")
    ls_str_results,  ls_pure_results  = exp_and_results(all_data, args.tree_count, args.max_layers_per_tree, args.seed, True, 3, args.other_split)
    agg_str_results, agg_pure_results = aggregate_results(args.other_split, nls_pure_results, ls_pure_results)

    nls_str_results = ["LINEAR SPLITTING RESULTS:"] + nls_str_results
    ls_str_results  = [args.other_split + " SPLITTING RESULTS:"] + ls_str_results
    agg_str_results = ["AGGREGATE RESULTS WITH LINEAR AND " + args.other_split + " SPLITTING:"] + agg_str_results
    full_results    = nls_str_results + ["\n"] + ls_str_results + ["\n"] + agg_str_results

    full_print = "\n".join(full_results)
    print(full_print)

    results_filepath = get_other_split_log_file_filepath(args.other_split, args.dataset_name, args.tree_count, args.max_layers_per_tree, args.seed)
